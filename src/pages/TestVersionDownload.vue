    ...
    unlisten_finished = await listen<DownloadFinished>("minecraft-download-finished", (e) => {
        ...
        if (e.payload.success) {
            get_instance(version_id.value)
                .then((info) => { instance_info.value = info; })
...
<template>
    <div class="page">
        <header class="page-head">
            <h2>新建实例</h2>
            <p class="page-desc">选择版本并配置启动参数，下载完成后将自动建立实例。</p>
        </header>

        <mdui-card variant="outlined" class="card">
            <div class="card-head">
                <mdui-icon-extension--outlined class="card-icon"></mdui-icon-extension--outlined>
                <div>
            ...
            <div class="field-grid">
                <mdui-text-field label="版本号" :value="version_id" @input="version_id = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="实例名称（留空=版本号）" :value="instance_name" @input="instance_name = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="分辨率宽" type="number" :value="width_str" @input="width_str = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="分辨率高" type="number" :value="height_str" @input="height_str = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="JVM 参数（空格/逗号分隔）" class="full" :value="jvm_str" @input="jvm_str = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="游戏参数（空格/逗号分隔）" class="full" :value="game_str" @input="game_str = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="启动命令前缀" :value="prefix_str" @input="prefix_str = ($event.target as any).value"></mdui-text-field>
                <mdui-text-field label="启动命令后缀" :value="suffix_str" @input="suffix_str = ($event.target as any).value"></mdui-text-field>
            </div>
            <mdui-divider></mdui-divider>
            <div class="card-actions">
                ...
                <mdui-button variant="tonal" @click="fetch_version_json()">查看 version.json</mdui-button>
            </div>
        </mdui-card>
        ...
        <mdui-collapse :value="open_sections">
            <mdui-collapse-item value="global">
                <mdui-card variant="outlined" slot="header" class="collapse-head">
                    <mdui-icon-settings--outlined class="card-icon"></mdui-icon-settings--outlined>
                    <span class="collapse-title">全局设置</span>
                </mdui-card>
                <div class="collapse-body">
                    <div class="field-grid">
                        <mdui-text-field label="镜像地址（留空为官方源）" :value="mirror_url" @input="mirror_url = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="并发数" type="number" :value="concurrency_str" @input="concurrency_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认分辨率宽" type="number" :value="g_width_str" @input="g_width_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认分辨率高" type="number" :value="g_height_str" @input="g_height_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认 JVM 参数" class="full" :value="g_jvm_str" @input="g_jvm_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认游戏参数" class="full" :value="g_game_str" @input="g_game_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认启动命令前缀" :value="g_prefix_str" @input="g_prefix_str = ($event.target as any).value"></mdui-text-field>
                        <mdui-text-field label="默认启动命令后缀" :value="g_suffix_str" @input="g_suffix_str = ($event.target as any).value"></mdui-text-field>
                    </div>
                    <div class="card-actions">
                        <mdui-button variant="tonal" @click="save_global_config()">保存全局配置</mdui-button>
                    </div>
                </div>
            </mdui-collapse-item>
        </mdui-collapse>
        ...
        <template v-if="instance_info">
            <mdui-card variant="filled" class="info-card">
                ...
                <mdui-list>
                    <mdui-list-item>
                        <div class="info-row"><span>名称</span><b>{{ instance_info.name }}</b></div>
                    </mdui-list-item>
                    <mdui-list-item>
                        <div class="info-row"><span>路径</span><b>{{ instance_info.path }}</b></div>
                    </mdui-list-item>
                    <mdui-list-item>
                        <div class="info-row"><span>创建时间</span><b>{{ instance_info.createdAt }}</b></div>
                    </mdui-list-item>
                    <mdui-list-item>
                        <div class="info-row"><span>分辨率</span><b>{{ instance_info.config.width ?? '-' }}×{{ instance_info.config.height ?? '-' }}</b></div>
                    </mdui-list-item>
                    <mdui-list-item>
                        <div class="info-row"><span>JVM 参数</span><b class="mono">{{ instance_info.config.jvmArgs.join(' ') || '-' }}</b></div>
                    </mdui-list-item>
                </mdui-list>
            </mdui-card>
        </template>
        ...
        <mdui-collapse :value="open_sections">
            <mdui-collapse-item value="raw">
                <mdui-card variant="outlined" slot="header" class="collapse-head">
                    <mdui-icon-code--outlined class="card-icon"></mdui-icon-code--outlined>
