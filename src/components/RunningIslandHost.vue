...
const {
    sessions, expandedId, dockClearance, logsBySid,
    focus_session, collapse, stop_session, destroy_session,
} = useRunningStore();
...
function morphTo(target: { left: number; top: number; width: number; height: number }) {
...
async function mountPanel(startTransform: string | null, startRadius: number, startOpacity: number) {
...
function animatePanel(keyframes: Keyframe[], opts: KeyframeAnimationOptions): Promise<void> {
    const p = panelEl.value;
    if (!p) return Promise.resolve();
    panelAnim = p.animate(keyframes, { ...opts, fill: 'forwards' });
    return panelAnim.finished.then(() => {
        p.style.opacity = '1';
        p.style.transform = '';
        p.style.borderRadius = `${PANEL_RADIUS}px`;
    }).catch(() => undefined);
...
function animateBackdrop(opacity: number, duration = 240) {
    ...
    return backdropAnim.finished.then(() => {
    ...
    if (!overlayOn.value) {
        ...
        if (p) {
            panelAnim = p.animate(
                ...
                { duration: 240, easing: 'cubic-bezier(.32,.72,.24,1)', fill: 'forwards' },
            );
        }
    }
    setInner(1, 30);
...
async function openFromPill(session: RunningSession) {
    ...
    await animatePanel(
        [
            { transform: morphTo(rect), borderRadius: `${rect.height / 2}px`, opacity: 0.4, offset: 0 },
            { transform: 'none', borderRadius: `${PANEL_RADIUS}px`, opacity: 1, offset: 1 },
        ],
...
async function switchTo(session: RunningSession) {
    ...
    await animatePanel(
        [
            { transform: 'none', borderRadius: `${PANEL_RADIUS}px`, opacity: 1, offset: 0 },
            { transform: morphTo(rect), borderRadius: `${rect.height / 2}px`, opacity: 0.6, offset: 1 },
        ],
...
async function requestCollapse() {
    ...
    await animatePanel(
        [
            { transform: 'none', borderRadius: `${PANEL_RADIUS}px`, opacity: 1, offset: 0 },
            { transform: morphTo(rect), borderRadius: `${rect.height / 2}px`, opacity: 0.5, offset: 1 },
        ],
...
onMounted(async () => {
    ...
    await init_session_events().catch((e) => console.error('订阅会话事件失败:', e));
    nextTick(() => {
        attachDockObserver();
        updateDockClearance();
    });
...
function statusHint(s: RunningSession) {
    ...
    if (s.status === 'exited') return s.message ?? `已退出（代码 ${s.exitCode ?? '?'}）`;
...
<template>
    <div>
        ...
        <div v-if="overlayOn" class="island-overlay">
            <div ref="backdropEl" class="island-backdrop" @click="queueCollapse"></div>
            <section
                ref="panelEl"
                class="island-panel"
                :style="{
                ...
                <div ref="innerEl" class="island-inner">
                    <template v-if="contentSession">
                        <header class="island-head">
                            <img class="island-avatar" :src="iconOf(contentSession)" alt="avatar" />
                            <div class="island-title-wrap">
                                <div class="island-title">{{ contentSession.instance.name }}</div>
                                <div class="island-sub">{{ contentSession.instance.versionId }}</div>
                            </div>
                            <span class="status-inner" :class="statusClass(contentSession.status)">
                                <span class="status-dot"></span>
                                {{ statusText(contentSession.status) }}
                            </span>
                            <div class="island-actions">
                                <template v-if="isTerminal(contentSession.status)">
                                    <mdui-button-icon title="销毁面板" @click="onDestroy(contentSession)">
                                ...
                                <template v-else>
                                    ...
                                    <mdui-button-icon title="结束会话" @click="onStop(contentSession)">
                        ...
                        <div class="island-stats">
                            ...
                            <div class="stat">
                                <div class="stat-num">
                                    {{ contentSession.status === 'running' ? '存活' : (contentSession.exitCode ?? '—') }}
                                </div>
                                <div class="stat-label">退出代码</div>
                            </div>
                            <div class="stat">
                                <div class="stat-num">{{ fmtElapsed(Date.now() - contentSession.launchedAt) }}</div>
                        ...
                        <div class="island-log">
                            <div class="log-head">
                                <span>守护监视 · 运行日志</span>
                                <span class="log-head-sub">{{ statusHint(contentSession) }}</span>
                            </div>
                            <div ref="logAreaEl" class="log-body">
                                ...
                                ></p>
                                <p v-if="(logsBySid[contentSession.id] ?? []).length === 0" class="log-empty">
                                    暂无输出，等待会话数据…
                                </p>
                            </div>
                        </div>
