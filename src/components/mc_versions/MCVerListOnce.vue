...
<template>
    <mdui-list-item class="card">
        <mdui-avatar :src="props.type == 'snapshot' ? '/src/assets/mc_icon_beta.png' : '/src/assets/mc_icon.png'"
            slot="icon" class="icon" />
        <div class="text">
            <p class="main-text"><b>{{ props.id }}</b></p>
            <sub><mdui-icon-alarm-add class="text-icon clock"></mdui-icon-alarm-add> {{ props.time }} |
                <mdui-icon-commit class="text-icon commit"></mdui-icon-commit> {{ props.type == 'snapshot' ? '快照' : '正式版'
                }}</sub>
        </div>
